...
class TestnetCredentialsDialog(QDialog):
    def __init__(self, parent: QWidget | None = None, api_key: str = "", api_secret: str = "") -> None:
        ...
        note.setStyleSheet("color:#7c5a08;font-weight:700;")
        root.addWidget(note)
...
class MainWindow(QMainWindow):
    """Compact Arabic V1 dashboard with WebSocket-driven price monitoring."""
    ...
    def _new_paper_provider() -> PaperOCOProvider:
        paper_orders = [o for o in sample_ocos() if o.symbol != "TUTUSDT"]
        return PaperOCOProvider(paper_orders, {"FIDAUSDT": Decimal("0.078210")}, {"FIDAUSDT": Decimal("0.000001")})
    ...
    def _build_ui(self) -> None:
        ...
        sub = QLabel("محرر OCO الآمن • V1")
        sub.setStyleSheet("color:#687581;font-size:13px;")
        ...
        self.connection = QLabel(); self.connection.setStyleSheet("font-weight:700;margin-left:5px;")
        header.addWidget(self.connection); root.addLayout(header)

        safety = QFrame(); safety.setStyleSheet("QFrame{background:#fff8e8;border:1px solid #efd79a;border-radius:6px;}")
        sr = QHBoxLayout(safety); sr.setContentsMargins(8,4,8,4)
        lock = QLabel("OCO المحدد:"); lock.setStyleSheet("font-weight:800;color:#7c5a08;")
        sr.addWidget(lock)
        self.target_label = QLabel("لا يوجد طلب محدد"); self.target_label.setStyleSheet("color:#5e4a16;")
        sr.addWidget(self.target_label, 1); sr.addWidget(QLabel("التعديل محلي حتى التفعيل")); root.addWidget(safety)
        ...
        orders = QGroupBox("الأوامر المفتوحة")
        ol = QVBoxLayout(orders); ol.setContentsMargins(6,6,6,6); ol.setSpacing(4)
        top = QHBoxLayout(); title = QLabel("اختر OCO واحدًا فقط"); title.setStyleSheet("font-weight:800;")
        top.addWidget(title); top.addStretch(); self.refresh_btn=QPushButton("تحديث"); self.refresh_btn.clicked.connect(lambda:self._refresh_orders(True)); top.addWidget(self.refresh_btn); ol.addLayout(top)
        self.orders = QTableWidget(0,6); self.orders.setHorizontalHeaderLabels(["List ID","العملة","الكمية","البيع","الاستوب","الحالة"])
        self.orders.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows); self.orders.setSelectionMode(QTableWidget.SelectionMode.SingleSelection); self.orders.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        self.orders.verticalHeader().setVisible(False); self.orders.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch); self.orders.itemSelectionChanged.connect(self._select_current)
        self.orders.setMinimumHeight(205); self.orders.setMaximumHeight(230); ol.addWidget(self.orders); grid.addWidget(orders,0,0)

        selected = QGroupBox("الطلب المحدد — حي")
        sl = QGridLayout(selected); sl.setContentsMargins(8,7,8,7); sl.setHorizontalSpacing(7); sl.setVerticalSpacing(5)
        ...
        trail=QGroupBox("تتبع الصعود التلقائي")
        tl=QGridLayout(trail); tl.setContentsMargins(8,7,8,7); tl.setHorizontalSpacing(7); tl.setVerticalSpacing(5)
        ...
        self.trail_enable=QPushButton("تشغيل التتبع"); self.trail_enable.clicked.connect(self._toggle_trail); tl.addWidget(self.trail_enable,1,0,1,2)
        self.trail_state=QLabel("متوقف"); self.trail_state.setStyleSheet("font-weight:800;"); tl.addWidget(self.trail_state,1,2,1,4)
        self.anchor_label=QLabel("مرجع: —    التالي: —    حي: —"); tl.addWidget(self.anchor_label,2,0,1,6); grid.addWidget(trail,1,0,1,2)

        editor=QGroupBox("تعديل سريع للـ OCO")
        el=QGridLayout(editor); el.setContentsMargins(8,7,8,7); el.setHorizontalSpacing(7); el.setVerticalSpacing(5)
        self.tp_edit=QLineEdit(); self.sl_edit=QLineEdit(); self.qty_edit=QLineEdit(); self.qty_edit.setReadOnly(True)
        el.addWidget(QLabel("سعر البيع"),0,0); el.addWidget(self.tp_edit,0,1); el.addWidget(QLabel("الكمية الأصلية"),0,2); el.addWidget(self.qty_edit,0,3)
        stoprow=QHBoxLayout(); stoprow.addWidget(self.sl_edit,1); self.max_stop_btn=QPushButton("MAX STOP"); self.max_stop_btn.clicked.connect(self._apply_max_stop); stoprow.addWidget(self.max_stop_btn)
        el.addWidget(QLabel("الاستوب"),1,0); el.addLayout(stoprow,1,1); self.activate_btn=QPushButton("تفعيل الاستبدال"); self.activate_btn.setObjectName("primary"); self.activate_btn.clicked.connect(self._activate); el.addWidget(self.activate_btn,1,2,1,2)
        self.state_label=QLabel("لا يوجد طلب محدد"); self.state_label.setStyleSheet("font-weight:700;"); el.addWidget(self.state_label,2,0,1,4); grid.addWidget(editor,2,0)
    ...
    def _switch_mode(self,mode:str)->None:
        ...
        if mode=="TESTNET":
            ...
            old=self.provider; self.provider=provider; self.service=OCOEditorService(provider); self.mode="TESTNET"; self._set_paper_controls_enabled(False); self._clear_selection_ui(); self._set_mode_visuals(); self._refresh_orders(False); getattr(old,"close",lambda:None)()
        else:
            old=self.provider; self.provider=self._new_paper_provider(); self.service=OCOEditorService(self.provider); self.mode="PAPER"; self._set_paper_controls_enabled(True); self._clear_selection_ui(); self._set_mode_visuals(); self._refresh_orders(False); getattr(old,"close",lambda:None)()
    ...
    def _on_price(self,price:Decimal)->None:
        ...
        try: tick=self.provider.get_tick_size(symbol); self.tick_size.setText(f"{tick:f}"); self.max_stop.setText(f"{highest_sell_stop_candidate(price,tick):f}")
        except Exception: pass
    ...
    def _toggle_trail(self)->None:
        if self.trail and self.trail.snapshot().enabled: self._disable_trail(); return
        if not self.service.original or not self.service.selection: QMessageBox.information(self,"التتبع","اختر OCO أولًا."); return
        try:
            settings=AutoTrailSettings.parse(self.trigger_edit.text(),self.tp_move_edit.text(),self.sl_move_edit.text()); live=self.provider.get_last_price(self.service.selection.symbol); self._disable_trail(); self.trail=AutoTrailEngine(self.provider,self._trail_event,self._trail_finished); self.trail.enable(self.service.original,settings,live); self.trail_state.setText("● يعمل — يستمع للسعر اللحظي"); self.trail_enable.setText("إيقاف التتبع"); self._log("تتبع",f"تشغيل على OCO {self.service.selection.order_list_id} من {live}"); self._on_price(live)
    ...
    def _worker_finished(self,payload:object)->None:
        ...
        if kind=="trail_done":
            ...
            if ok:
                new_id=result.get("orderListId") if isinstance(result,dict) else None; self._log("نجاح",message); self._refresh_orders(True)
                if new_id:
                    try:
                        draft=self.service.select(int(new_id)); self.tp_edit.setText(str(draft.values.get("abovePrice") or draft.values.get("leg1.price") or "")); self.sl_edit.setText(str(draft.values.get("belowStopPrice") or draft.values.get("leg2.stopPrice") or "")); self.qty_edit.setText(str(draft.values.get("quantity") or draft.values.get("leg1.quantity") or "")); self.state_label.setText("التتبع يعمل على أحدث OCO")
                    except Exception: pass
    ...
    def _force_execute(self,leg:str)->None:
        if self.mode!="PAPER" or not isinstance(self.provider,PaperOCOProvider) or not self.service.selection: return
        try:
            oid=self.service.selection.order_list_id; price=self.provider.get_last_price(self.service.selection.symbol); self.provider.force_execute(oid,leg,price); self._log("تنفيذ",f"محاكاة {leg} على OCO {oid}"); self._refresh_orders()
