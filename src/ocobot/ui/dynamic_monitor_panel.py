...
QLineEdit#percentInput {{ background:{CARD_ALT}; color:{TEXT}; border:1px solid #154a72; border-radius:6px; padding:7px 9px; font-size:11px; font-weight:700; }}
QLineEdit#percentInput:focus {{ border:1px solid {BLUE}; }}
QPushButton#monitorAction {{ background:{BLUE}; color:#ffffff; border:0; border-radius:7px; padding:8px 13px; font-size:10px; font-weight:800; }}
QPushButton#monitorStop {{ background:#2a1420; color:#ff8ca0; border:1px solid {RED}; border-radius:7px; padding:8px 13px; font-size:10px; font-weight:800; }}
QLabel#state {{ color:{MUTED}; font-size:10px; font-weight:800; }}
QFrame#derived {{ background:#041a25; border:1px solid #0d4254; border-radius:7px; }}
QLabel#derivedLabel {{ color:#81a4bc; font-size:10px; }}
QLabel#derivedValueTP {{ color:{GREEN}; font-size:12px; font-weight:800; }}
QLabel#derivedValueSL {{ color:#ff8ca0; font-size:12px; font-weight:800; }}
QLabel#formulaNote {{ color:#91aec0; font-size:9px; }}
"""
...
class DynamicMonitorPanel(QFrame):
    ...
    def _build_ui(self) -> None:
        ...
        derived_grid.setContentsMargins(9, 7, 9, 7)
        self.live_value = self._derived(derived_grid, 0, 0, "Live Price", "—", "tp")
        self.tp_value = self._derived(derived_grid, 0, 1, "Calculated TP Sale Price", "—", "tp")
    ...
    def _field(grid: QGridLayout, row: int, col: int, label: str, value: str) -> QLineEdit:
        ...
        edit.setAlignment(__import__("PySide6.QtCore", fromlist=["Qt"]).Qt.AlignmentFlag.AlignCenter)
